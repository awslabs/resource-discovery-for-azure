...
- **Scans all subscriptions in a tenant by default**
...
- Access at [Azure Cloud Shell](https://shell.azure.com)
...
- **Local PowerShell:** Run `az login` and `Connect-AzAccount` before running the script. `Run-AllSubscriptions.ps1` prompts you to sign in once and then reuses the session for every subscription.
...
By default, the tool scans **all subscriptions** in the specified tenant, generating a separate inventory report for each one and bundling them into a single ZIP for delivery.
...
1. Authenticate once for the entire session
2. Enumerate all subscriptions in the tenant
...
```powershell
./ResourceInventory.ps1 -ReportName "CompanyName" -SubscriptionID "12345678-1234-1234-1234-123456789012" -ConcurrencyLimit 6
```
...
When running across all subscriptions, each subscription produces its own set of timestamped reports. After all subscriptions are processed, the wrapper bundles the per-subscription ZIPs into a single `AllSubscriptions_ResourcesReport_<timestamp>.zip` for easy delivery.
...
### Run-AllSubscriptions.ps1 (Default)
...
### ResourceInventory.ps1 (Single Subscription)
...
|-----------|------|-------------|---------|---------|
| ConcurrencyLimit | Integer | Parallel processing limit | 6 | `-ConcurrencyLimit 8` |
...
If you download the ZIP file, Windows may mark the files as blocked for security reasons (a known issue with GitHub downloads). Run the following PowerShell command to unblock all files:

```powershell
Get-ChildItem -Path . -Recurse | Unblock-File
```

After unblocking, you can run the script from the folder.

**Authentication Errors:**

- Ensure you have the required Azure roles assigned
- For local environments, run `az login` before running the script
- Multiple authentication prompts may appear due to parallel processing
- When using `Run-AllSubscriptions.ps1`, you only need to authenticate once

**Performance Issues:**

- Reduce `ConcurrencyLimit` if experiencing timeouts
- Use `SkipConsumption` to speed up processing. This is not recommended, as it greatly reduces the usefulness of the report.
- Consider targeting specific subscriptions or resource groups for testing
